...
func TestRecoverTree(t *testing.T) {
	utils.TestWarp("99 测试用例", func() {
		...
		testTemp := func() {
			...
			p := CreateTree(params1)

			RecoverTree(p)

			utils.TestCondition(t, desc, IsSameTree(p, CreateTree(res)))
		}

		params1 = []int{1, 3, math.MinInt, math.MinInt, 2}
		res = []int{3, 1, math.MinInt, math.MinInt, 2}
		testTemp()

		params1 = []int{3, 1, 4, math.MinInt, math.MinInt, 2}
		res = []int{2, 1, 4, math.MinInt, math.MinInt, 3}
		testTemp()
	})
}
